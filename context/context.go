...
//
// [WithCancelCause] 関数は [CancelCauseFunc] を返します。これは
// エラーを受け取り、それをキャンセル原因として記録します。[WithDeadlineCause]
// と [WithTimeoutCause] は、期限切れのときに使用する原因を受け取ります。
// キャンセルされたコンテキスト、またはその任意の子に対して [Cause] を呼び出すと、
// その原因を取得できます。原因が指定されていない場合、Cause(ctx) は
// ctx.Err() と同じ値を返します。
//
