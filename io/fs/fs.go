...
//
// openに渡されるパス名は、UTF-8エンコードされた、
// ルートを持たない、スラッシュで区切られたパス要素のシーケンスです。例："x/y/z"。
// パス名には"."や".."または空文字列の要素を含んではいけません。
// ただし、特別なケースとして、ルートディレクトリには"."という名前を使用できます。
// パスはスラッシュで始まったり終わったりしてはいけません："/x"や"x/"は無効です。
//
