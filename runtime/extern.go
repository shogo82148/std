...
これは、これらの名前付き変数を設定するname=valペアのカンマ区切りリストです。
	...
	panicnil: panicnil=1を設定すると、nilインターフェース値または型なしのnilでpanicを呼び出した時のランタイムエラーが無効になります。
	...
	tracefpunwindoff: tracefpunwindoff=1を設定すると、実行トレーサーと
	ブロック・ミューテックスプロファイラーが、フレームポインタアンワインディングの代わりに
	ランタイムのデフォルトスタックアンワインダーを使用することを強制します。これにより
	オーバーヘッドが増加しますが、フレームポインタアンワインディングによって引き起こされる
	予期しないリグレッションの回避策やデバッグに役立つ可能性があります。
...
ランタイムがバイナリをsetuid/setgidのようなものと判断した場合、主に2つのことを行います：
  - GOTRACEBACK環境変数の値が 'none' に設定されます。
  - プログラムを終了させるシグナルを受信した場合、またはプログラムが
    GOTRACEBACKの値を上書きするような回復不能なパニックに遭遇した場合、
    ゴルーチンスタック、レジスタ、およびその他のメモリ関連情報は省略されます。

さらにUnixプラットフォームでは、Goランタイムは標準入出力ファイルディスクリプタ
（0、1、2）が開いているかどうかを自動的にチェックします。いずれかが閉じられている場合、
/dev/null を指すように開かれます。
