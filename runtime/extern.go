...
これは、これらの名前付き変数を設定するname=valペアのカンマ区切りリストです。
	...
	他のプラットフォームでは影響はありません。disablethpは、Goのバージョン1.21以前との互換性を保つためのもので、
	これにより大幅なメモリの過剰使用を引き起こす可能性のあるLinuxカーネルのデフォルトを回避する動作が停止しました。
	詳細は https://go.dev/issue/64332 を参照してください。この設定は将来のリリースで削除される予定なので、
	その時点までに運用者はLinuxの設定を自分のニーズに合わせて調整する必要があります。
	詳細は https://go.dev/doc/gc-guide#Linux_transparent_huge_pages を参照してください。

	dontfreezetheworld: デフォルトでは、致命的なパニックまたはスローの開始は
	"世界をフリーズ"させ、すべてのスレッドを事前に停止させてすべての実行中の
	...
	自然にスケジューラに入るゴルーチンは依然として停止します。これはランタイムスケジューラの
	デバッグ時に有用で、freezetheworldはスケジューラの状態を乱すため、問題を隠す可能性があります。
