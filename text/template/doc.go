...
- Go構文のブール値、文字列、文字、整数、浮動小数点数、虚数または複素数定数。
  これらはGoの無名定数のように振る舞います。Goと同様に、大きな整数定数が
  割り当てられたときや関数に渡されたときにオーバーフローするかどうかは、
  ホストマシンのintが32ビットか64ビットかに依存する場合があります。
- Goの無名nilを表すキーワードnil。
- 文字 '.'（ピリオド）:
  .
  結果はdotの値です。
- 変数名。これはドル記号で始まる英数字の文字列（空である可能性もあります）です。
  例えば
    $piOver2
  または
    $
  結果は変数の値です。変数については後述します。
- データのフィールド名。これは構造体でなければならず、ピリオドで始まります。
  例えば
    .Field
  結果はフィールドの値です。フィールドの呼び出しは連鎖できます:
    .Field1.Field2
  フィールドは変数上でも評価でき、連鎖も可能です:
    $x.Field1.Field2
- データのキー名。これはマップでなければならず、ピリオドで始まります。
  例えば
    .Key
  結果はキーでインデックスされたマップ要素の値です。
  キーの呼び出しは連鎖でき、フィールドと任意の深さで組み合わせることができます:
    .Field1.Key1.Field2.Key2
  キーは英数字の識別子でなければなりませんが、フィールド名とは異なり、
  大文字で始まる必要はありません。キーは変数上でも評価でき、連鎖も可能です:
    $x.key1.key2
- データのniladicメソッド名。これはピリオドで始まります。
  例えば
    .Method
  結果はメソッドをレシーバとして呼び出した値、dot.Method()です。
  このようなメソッドは1つの戻り値（任意の型）または2つの戻り値を持ち、
  2つ目の戻り値がエラーである必要があります。2つの戻り値があり、
  戻り値のエラーがnilでない場合、実行は終了し、エラーがExecuteの値として
  呼び出し元に返されます。メソッドの呼び出しは連鎖でき、フィールドやキーと
  任意の深さで組み合わせることができます:
    .Field1.Key1.Method1.Field2.Key2.Method2
  メソッドは変数上でも評価でき、連鎖も可能です:
    $x.Method1.Field
- niladic関数の名前。例えば
    fun
  結果は関数を呼び出した値、fun()です。戻り値の型と値はメソッドと同様に振る舞います。
  関数と関数名については後述します。
- 上記のいずれかの括弧で囲まれたインスタンス。結果はフィールドまたはマップキーの
  呼び出しによってアクセスできます。
    print (.F1 arg1) (.F2 arg2)
    (.StructValuedMethod "arg").Field
引数は任意の型に評価される可能性があります。ポインタである場合、
実装は必要に応じて自動的に基底型に間接参照します。
評価が関数値を生成する場合（例えば、構造体の関数値フィールド）、
関数は自動的に呼び出されませんが、ifアクションなどの真偽値として使用できます。
呼び出すには、以下で定義されているcall関数を使用します。
パイプライン
パイプラインは「コマンド」の連鎖されたシーケンスである可能性があります。
コマンドは単純な値（引数）または関数やメソッドの呼び出しであり、
複数の引数を持つこともあります:
	Argument
		結果は引数の評価値です。
	.Method [Argument...]
