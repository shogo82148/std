...
// 圧縮効率を犠牲にします。
//
// レベルが[-2、9]の範囲内である場合、返されるエラーはnilになります。
// それ以外の場合、返されるエラーはnilではありません。
//
// wに書き込まれた正確なバイト数はGo 1の互換性保証の対象外です。
// テストを含む呼び出し元は、正確に書き込まれたバイト数に依存してはいけません。
func NewWriter(w io.Writer, level int) (*Writer, error)

// NewWriterDictは[NewWriter]に似ていますが、新しい
// [Writer]をプリセット辞書で初期化します。返される[Writer]は
// あたかも辞書が圧縮出力を生成せずに書き込まれたかのように動作します。
// wに書き込まれた圧縮データは、同じ辞書で初期化されたリーダーによってのみ
// 解凍できます（[NewReaderDict]を参照）。
//
// wに書き込まれた正確なバイト数はGo 1の互換性保証の対象外です。
// テストを含む呼び出し元は、正確に書き込まれたバイト数に依存してはいけません。
