...
//
// 返される cname は、最初の SRV 応答レコードの所有者名です。
// これは通常、構築された DNS 名 (_service._proto.name) ですが、
// CNAME レコードが問い合わせを別の名前にリダイレクトする場合は異なることがあります。
//
// 返されるサービス名は、適切にフォーマットされたプレゼンテーション形式の
// ドメイン名であることが検証されます。応答に無効な名前が含まれている場合、
// それらのレコードは除外され、残りの結果がある場合は、それと一緒に
// エラーが返されます。
