...
//		linux/amd64、linux/arm64、freebsd/amd64でのみサポートされています
//		そしてホストCコンパイラとしてClang/LLVMのみ。
//		すべてのプラットフォームでPIEビルドモードが使用されます（linux/amd64を除く）。
//	-asan
//		アドレスサニタイザとの相互運用を有効にします。
//		linux/arm64、linux/amd64でのみサポートされています。
//		GCC 7以降、またはClang/LLVM 9以降がある場合にのみ、linux/amd64またはlinux/arm64でサポートされています。
//	-cover
//		コードカバレッジ計測を有効にします。
...
// ファイルの上部近く、空行や他の行コメントのみに先行して現れる必要があります。
// これらのルールは、Goファイルではビルド制約がパッケージ節の前に現れなければならないことを意味します。
//
...
// # レガシーGOPATH go get
//
// 'go get'コマンドは、goコマンドがモジュール対応モードで動作しているか、
// レガシーGOPATHモードで動作しているかによって、振る舞いが変わります。
// このヘルプテキストは、モジュール対応モードでも'go help gopath-get'としてアクセス可能で、
// レガシーGOPATHモードでの'go get'の動作を説明しています。
//
// 使用法: go get [-d] [-f] [-t] [-u] [-v] [-fix] [build flags] [packages]
//
// Getは、インポートパスによって指定されたパッケージとその依存関係をダウンロードします。
// その後、指定されたパッケージをインストールします。これは'go install'と同様です。
//
// -dフラグは、getにパッケージのダウンロード後に停止するよう指示します。
// つまり、getにパッケージをインストールしないよう指示します。
//
// -fフラグは、-uが設定されている場合のみ有効で、get -uに対して、
// 各パッケージがそのインポートパスから暗示されるソースコントロールリポジトリから
// チェックアウトされたことを確認しないよう強制します。これは、ソースがオリジナルのローカルフォークである場合に便利です。
//
// -fixフラグは、getに依存関係を解決したりコードをビルドしたりする前に、
// ダウンロードしたパッケージでfixツールを実行するよう指示します。
//
// -tフラグは、getに指定されたパッケージのテストをビルドするために必要なパッケージもダウンロードするよう指示します。
//
// -uフラグは、getにネットワークを使用して指定されたパッケージとその依存関係を更新するよう指示します。
// デフォルトでは、getは欠落しているパッケージをチェックアウトするためにネットワークを使用しますが、
// 既存のパッケージの更新を探すためには使用しません。
//
// -vフラグは、詳細な進行状況とデバッグ出力を有効にします。
//
// Getはまた、インストールを制御するためのビルドフラグも受け入れます。詳細は'go help build'を参照してください。
//
// 新しいパッケージをチェックアウトするとき、getはターゲットディレクトリ
// GOPATH/src/<import-path>を作成します。もしGOPATHが複数のエントリを含んでいる場合、
// getは最初のものを使用します。詳細は 'go help gopath' を参照してください。
//
// パッケージをチェックアウトまたは更新するとき、getはローカルにインストールされたGoのバージョンと
// 一致するブランチまたはタグを探します。最も重要なルールは、ローカルのインストールがバージョン "go1" を
// 実行している場合、getは "go1" という名前のブランチまたはタグを探します。そのようなバージョンが存在しない場合、
// パッケージのデフォルトブランチを取得します。
//
// go getがGitリポジトリをチェックアウトまたは更新するとき、
// それはまた、リポジトリによって参照される任意のgitサブモジュールも更新します。
//
...
// パッケージの指定についての詳細は、'go help packages'を参照してください。
//
// 'go get'がダウンロードするソースコードを見つける方法についての詳細は、
// 'go help importpath'を参照してください。
//
// このテキストは、ソースコードと依存関係を管理するためにGOPATHを使用しているときのgetの動作を説明しています。
// 代わりにgoコマンドがモジュール対応モードで動作している場合、
// getのフラグと効果の詳細が変わり、'go help get'も変わります。
// 'go help modules'と'go help module-get'を参照してください。
//
