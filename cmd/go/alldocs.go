...
// バイナリがどこにインストールされるかについての詳細は、'go help gopath'を実行してください。
// GoとC/C++の間の呼び出しについての詳細は、'go help c'を実行してください。
// プロジェクトの編成についての詳細は、'go help modules'を実行してください。
//
// 注記: go buildはプロジェクトを編成するための特定の規約に従います：
// 主にgoモジュール（'go help modules'を参照）をサポートしていますが、
// 代替のGOPATHモード（'go help gopath'を参照）もサポートしています。
// ただし、すべてのプロジェクトがこれらの規約に従うことができるわけではありません。
// 独自の規約を持つか、別のソフトウェアビルドシステムを使用するインストールは、
// ビルドツールのオーバーヘッドと設計上の決定の一部を避けるために、
// 'go tool compile'や'go tool link'などの低レベルの呼び出しを使用することを選択できます。
//
...
//		-cmd
//			コマンド（パッケージmain）を通常のパッケージと同様に扱います。
//			それ以外の場合、パッケージのトップレベルのドキュメントを表示するときに
//			パッケージmainのエクスポートされたシンボルが非表示になります。
//		-ex
//			実行可能な例を含めます。
//		-http
//			HTML docsをHTTP経由で提供します。
//		-short
//			各シンボルの1行表示。-allと組み合わせることはできません。
//		-src
...
//
//	  -diff
//		各修正を適用する代わりに、パッチをunified diff形式で表示します。
//		diffが空でない場合、非ゼロのステータスで終了します。
//
...
//
// -toolフラグは、リストされた各パッケージに対してgo.modに対応するtool行を
// 追加するようにgoに指示します。-toolを@noneとともに使用すると、その行が削除されます。
// 詳細については 'go help tool' を参照してください。
//
...
//
// -go=version フラグは、想定される Go 言語のバージョンを設定します。
// このフラグは主に、Go のバージョン依存関係を理解するツール向けです。
// "1.26" や "1.26.2" のようなバージョンを取ります。
// "none" を使うと go ディレクティブを削除します。
// ユーザーは 'go get go@version' を使うべきです。
//
// -toolchain=name フラグは、使用する Go ツールチェーンを設定します。
// このフラグは主に、Go のバージョン依存関係を理解するツール向けです。
// "go1.26" や "go1.26.2" のようなツールチェーン名を取ります。
// "none" を使うと toolchain ディレクティブを削除します。
// ユーザーは 'go get toolchain@version' を使うべきです。
//
...
//	)
//
// useディレクティブはワークスペースのメインモジュールのセットに含めるモジュールを指定します。
// useディレクティブの引数はモジュールのgo.modファイルを含むディレクトリです。
// goコマンドはuseパスをモジュールディレクトリに照合する際にシンボリックリンクを解決しないため、
// ディレクトリへのシンボリックリンクはそのターゲットと互換性がありません。
//
...
// 編集フラグは一連の編集操作を指定します。
//
// -fmt フラグは、他の変更を加えずに go.work ファイルを再整形します。
// この再整形は、go.work ファイルを使用または書き換える他の変更でも暗黙に行われます。
// このフラグが必要になるのは、'go work edit -fmt' のように、他のフラグが
// 指定されていない場合だけです。
//
// -godebug=key=value フラグは godebug key=value 行を追加し、
// 与えられたキーを持つ既存の godebug 行を置き換えます。
...
// -go=version フラグは、想定される Go 言語のバージョンを設定します。
// "1.26" や "1.26.2" のようなバージョンを取ります。
// バージョンに "none" を使うと go ディレクティブを削除します。
//
// -toolchain=name フラグは、使用する Go ツールチェーンを設定します。
// "go1.26" や "go1.26.2" のようなツールチェーン名を取ります。
// 名前に "none" を使うと toolchain ディレクティブを削除します。
//
...
//
// -r フラグは引数ディレクトリ内のモジュールを再帰的に検索し、
// use コマンドは各ディレクトリが引数として指定されたかのように動作します。
// -r が使用される場合、引数ツリー内のディレクトリへのシンボリックリンクは無視されます。
//
// goコマンドはシンボリックリンクを解決せずにuseパスをモジュールディレクトリに照合します。
// ディレクトリへのシンボリックリンクを指定するuseディレクティブは、
// シンボリックリンクのターゲットを指定するものとは互換性がありません。
//
// 詳細情報については、https://go.dev/ref/mod#workspaces のワークスペースリファレンスを参照してください。
//
// # Make vendored copy of dependencies
...
//
// goコマンドはサブプロセスの環境変数$PATHの先頭に$GOROOT/binを追加します。
// これにより、'go'コマンドを実行するサブプロセスが親プロセスと同じ'go'を使用するようになります。
//
...
//
// テストバイナリのビルドの一環として、go testは重大な問題を特定するために
// パッケージとそのテストソースファイルに対してgo vetを実行します。go vetが
// 何らかの問題を見つけた場合、go testはそれらを報告し、テストバイナリを
// 実行しません。使用されるのは、デフォルトのgo vetチェックのうち
// 信頼度の高い一部のみです。そのサブセットは、atomic、bools、buildtag、
// directive、errorsas、ifaceassert、nilfunc、printf、stdversion、
// stringintconv、およびtestsです。
// これらや他のvetテストのドキュメントは、
// "go doc cmd/vet"で確認できます。go vetの実行を無効にするには、-vet=offフラグを使用します。
// すべてのチェックを実行するには、-vet=allフラグを使用します。
//
...
//
// Goにはいくつかの組み込みツールが付属しており、現在のモジュールのgo.modで
// 追加のツールを定義できます。'go get -tool' を使用して、現在のモジュールの
// go.modファイルに追加のツールを定義できます。詳細については 'go help get' を参照してください。
//
// コマンドは、toolディレクティブで宣言されたツールへの完全なパッケージパスを使用して
// 指定できます。メジャーバージョンサフィックスを除いたパッケージパスの最後の要素である
// ツールのデフォルトバイナリ名も、宣言されたツールの中で一意であれば使用できます。
//
...
//
// goコマンドはtoolディレクティブ経由で実行されるコマンドの環境変数$PATHの先頭に
// $GOROOT/binを追加します。これにより、親の'go tool'と同じ'go'を使用するようになります。
//
// ビルドフラグについての詳細は、'go help build'を参照してください。
//
...
//
// GOPATH環境変数は、GOMODCACHE およびGOBINでそれぞれ
// オーバーライドされない限り、モジュールキャッシュと
// インストールされたバイナリのデフォルト保存場所を変更するために使用されます。
//
// ほとんどのユーザーは明示的にGOPATHを設定する必要がありません。
// 環境変数が設定されていない場合、GOPATHはデフォルトでユーザーの
// ホームディレクトリ内の「go」という名前のサブディレクトリになります
// （UnixではHOME/go、Windowsでは%USERPROFILE%\go）、
// ただし、そのディレクトリにGo配布版が含まれていない場合です。
// 現在のGOPATHを確認するには「go env GOPATH」を実行してください。
//
// モジュールキャッシュはGOPATH/pkg/modで指定されたディレクトリに保存されます。
// GOMODCACHE が設定されている場合は、
// モジュールキャッシュを保存するディレクトリとして使用されます。
//
// 「go install」を使用してインストールされた実行ファイルは、
// GOPATH/binで指定されたディレクトリに、またはGOBINが設定されている場合は
// GOBINに配置されます。
//
// # GOPATH mode
//
// GOPATH環境変数はまた、go コマンドのレガシー動作によって使用されます。
// GOPATHモードと呼ばれるこのツールチェーンでは、Go 1.11でモジュールが導入される前に作成された
// 古いプロジェクトでモジュールに更新されたことのないプロジェクトが
// 引き続きビルドされることを許可します。
//
// GOPATHモードは、モジュールが無効な場合に有効になります。
// GO111MODULE=offの場合、またはGO111MODULE=autoで
// 作業ディレクトリがモジュールまたはワークスペース内にない場合です。
//
// GOPATHモードでは、パッケージはGOPATH環境変数を使用して配置されます。
// これは検索するパスのリストを指定します：
// Unixでは、値はコロン区切りの文字列です。
// Windowsでは、値はセミコロン区切りの文字列です。
// Plan 9では、値はリストです。
...
//
// GOPATHモードでは、「vendor」という名前のディレクトリ以下のコードは
// 「vendor」の親を根とするディレクトリツリー内のコードによってのみ
// インポート可能です。また、vendor要素までとvendor要素を含む
// プレフィックスを省略したインポートパスのみを使用できます。
//
// 以下に前のセクションの例を示しますが、
...
//
// GOPATHモードの vendorディレクトリ内のコードは、
// ソースツリーの深い場所にあると、より高いディレクトリ内のコードをシャドウします。
// fooをルートとするサブツリー内では、「crash/bang」のインポートは
// トップレベルの「crash/bang」ではなく「foo/vendor/crash/bang」に解決されます。
//
...
//
// モジュールモードでのvendoringの詳細については
// https://go.dev/ref/mod#vendoring を参照してください。
//
...
// # Import path syntax
//
// インポートパスはパッケージを一意に識別して見つけるために使用されます。
// 一般的に、インポートパスは標準ライブラリパッケージ
// （"unicode/utf8"など）またはモジュール内にあるパッケージを表します
// （詳細については 'go help modules' を参照してください）。
//
// 標準ライブラリは、最初の要素にドットを含まないすべてのインポートパスを
// 自身のパッケージ用に予約しています。モジュールのインポートパスの選択については
// 以下の「Fully-qualified import paths」を参照してください。
// 以下の名前は、ローカル作業時やチュートリアル、例、テストコードで
// 短いモジュール名として使用するために予約されています。
//
// - "test"
// - "example"
//
// # Internal packages
//
// 「internal」という名前のディレクトリ内またはその下にあるコードは、
// 「internal」ディレクトリより上に同じインポートパスを共有するコードのみによって
// インポート可能です。
// モジュール example.com/m のディレクトリレイアウトの例を次に示します：
//
//	/home/user/modules/m/
...
//	                y.go
//
// z.go内のコードは「example.com/m/foo/internal/baz」としてインポートされますが、
// そのインポートステートメントはインポートパスプレフィックス
// 「example.com/m/foo」を持つパッケージ内でのみ表示されます。
// パッケージ「example.com/m/foo」、「example.com/m/foo/bar」、および
...
//
// 標準ライブラリに属さないパッケージの完全修飾インポートパスは、
// そのパッケージが属するモジュールのパスで始まります。
// モジュールのパスはモジュールのソースコードをどこから取得するかを示します。
// 完全なインポートパスは、モジュールパスとモジュール内のパッケージの
// 相対ディレクトリパスを結合することで形成されます。例：
//
//	/home/user/modules/m/
//	        go.mod                 ("module example.com/m" を宣言)
//	        crash/
//	            bang/              ("example.com/m/crash/bang" としてインポート可能)
//	                b.go
//	        foo/                   ("example.com/m/foo" としてインポート可能)
//	            f.go
//	            bar/               ("example.com/m/foo/bar" としてインポート可能)
//	                x.go
//
// 最初の要素にドットを含まないインポートパスは標準ライブラリによって予約されているため、
// モジュールパス（すべてのインポートパスのプレフィックスとなる）はドットを含む要素で
// 始まる必要があります（例："github.com/user/repo" や "example.com/project"）。
// モジュールパスはコードホスティングサービスを直接指すか、
// またはHTMLメタタグでコードホスティングサービスを指すカスタムアドレスを指すことができます。
// モジュールはドキュメント用に "example"、テスト用に "test" という予約名も使用できます。
// これらのモジュールはgoコマンドでフェッチできません。
//
// 一般的なコードホスティングサイトでホストされているモジュールに属するインポートパスには特別な構文があります：
//
...
//
// 他のサーバーでホストされているモジュールの場合、インポートパスは
// バージョン管理システムの種類で修飾されるか、goツールが
// https/httpでインポートパスを動的に取得して、
// HTMLの<meta>タグからコードが存在する場所を発見することができます。
//
// コードの場所を宣言するために、形式
//
//	repository.vcs/path
...
//
// vcs は "fossil"、"git"、"hg"、"svn" のいずれかです。
//
...
// そのバリアントはコンテンツ値で「mod」をvcsとして使用します。例えば：
//
...
//	go <action> [packages]
//
// 通常、[packages] はパッケージパターンのリストで、以下のいくつかの形式をとることができます：
//
//   - ファイルシステムのディレクトリへの相対パスまたは絶対パス。
//     "..." ワイルドカード要素を含めることができます。
//   - インポートパス。"..." ワイルドカード要素を含めることができます。
//   - パッケージのセットに展開される予約済み名
//   - ファイルのリスト
//
// インポートパスが指定されていない場合、アクションは現在のディレクトリ内のパッケージに適用されます。
//
// ファイルシステムパスまたはインポートパス内の "..." 要素は
// 0個以上のパス要素にマッチするように展開されます。
// 具体的なルールは以下に説明します。
//
// # File system patterns
//
// UnixのようなOSでは / から始まるファイルシステムのルート、
// またはWindowsでは C: のようなボリューム名で始まるパターンは
// 絶対ファイルシステムパスとして解釈されます。
// "." または ".." 要素で始まるパターンは相対ファイルシステムパスとして解釈されます。
// ファイルシステムパスは指定したディレクトリ内のパッケージを示します。
//
// 相対パスはコマンドラインでの省略形として使用できます。
// "unicode" としてインポートされるコードのディレクトリで作業していて
// "unicode/utf8" のテストを実行したい場合、フルパスを指定する代わりに
// "go test ./utf8" と入力できます。
// 同様に逆の場合、"unicode/utf8" ディレクトリから "go test .." を実行すると
// "unicode" がテストされます。"go test ./..." のような相対パターンも使用でき、
// すべてのサブディレクトリをテストします。
//
// "..." ワイルドカードで展開されるファイルシステムパターンは以下を除外します：
//
// - "vendor" という名前のディレクトリ
// - "testdata" という名前のディレクトリ
// - "_" または "." で始まる名前のファイルおよびディレクトリ
// - go.mod ファイルを含むディレクトリ
// - モジュールの go.mod ファイルの ignore ディレクティブにマッチするディレクトリ
//
// これらはプレフィックスで直接指定するか、そのディレクトリに移動することで
// 含めることができます。例えば、"./..." は "./testdata/foo" パッケージにマッチしませんが、
// "./testdata/..." はマッチします。
//
// go.mod ファイルの存在で示される別のgoモジュールを含むディレクトリは、
// 作業ディレクトリをそのモジュール内に変更することでのみマッチできます。
//
...
//
// - "work"はメインモジュール（またはワークスペースモジュール）内のすべてのパッケージに展開されます。
//
// - "tool"は、現在のモジュールのgo.modファイルで定義されたツールに展開されます。
//
// - "all"はメインモジュール（またはワークスペースモジュール）内のすべてのパッケージと
// それらの依存関係（それらのテストに必要な依存関係を含む）に展開されます。
// レガシーGOPATHモードでは、"all"はすべてのGOPATHツリーで見つかったすべてのパッケージに展開されます。
//
// - "std"は標準ライブラリのすべてのパッケージと
// それらの内部ライブラリに展開されます。
//
...
// # List of .go files
//
// パターンが完全なパッケージではなくGoファイルのリストである場合、
// goコマンドは指定されたファイルのみを含む "command-line-arguments" という名前の
// 仮想パッケージを生成します。多くの場合、これはエラーになります
// （例："go build main.go" や "go build *.go"）。
// 代わりに "go build ." のように完全なパッケージ（ディレクトリ）を
// 対象にすることを推奨します。
//
// # Package names
//
// パッケージはインポートパスによって識別されます。
// 標準ライブラリのパッケージのインポートパスは "$GOROOT/src" 以下の
// 相対パスを使用します。
// その他のすべてのパッケージのインポートパスは、モジュール名と
// モジュール内の相対ディレクトリパスの組み合わせです。
// プログラム内では、すべてのパッケージは一意のインポートパスで識別される必要があります。
//
// パッケージには名前もあり、.goファイル内で "package" キーワードで宣言され、
// 別のパッケージにインポートされる際の識別子として使用されます。慣例として、
// インポート可能なパッケージの名前はインポートパスの最後の要素（通常はパッケージを
// 含むディレクトリ名）と一致します。
//
// パッケージ名はモジュール内で一意である必要はありませんが、
// 同じ名前を持つパッケージは、一方を別の名前にエイリアスしない限り
// 一緒にインポートできません。
//
// goコマンドは主にディレクトリを対象に動作するため、
// ディレクトリ内のテスト以外のすべての.goファイル（サブディレクトリを除く）は
// 同じパッケージ宣言を共有する必要があります。
// テストファイルはパッケージ宣言に "_test" サフィックスを付けることができます。
// これらのファイルのテストは別のパッケージとしてコンパイルされ、
// 対応するパッケージのエクスポートされていない識別子にはアクセスできません。
// 詳細については "go help test" および "go help testflag" を参照してください。
//
// 以下のパッケージ名は特別な意味を持ちます：
//
// - "main"はスタンドアロン実行可能ファイルのトップレベルパッケージを示します。
// "main" パッケージはインポートできません。
//
// - "documentation"はディレクトリ内の非Goプログラムのドキュメントを示します。
// documentation パッケージ内のファイルはgoコマンドによって無視されます。
//
// - "*_test.go" ファイルの "_test" サフィックス。これらは同じ場所にあるパッケージの
// エクスポートされた識別子にのみアクセスできる別のテストパッケージを形成します。
// 詳細については "go doc testing" を参照してください。
//
// インポートパスの詳細については "go help importpath" を参照してください。
//
...
//
// goコマンドは、gitなどのバージョン管理コマンドを実行して、
// インポートされたコードをダウンロードできます。この機能は分散型
// Goパッケージエコシステムに重要です。そこではコードは任意のサーバーから
// インポートできますが、悪意のあるサーバーが呼び出されたバージョン管理
// コマンドを意図しないコードを実行させる方法を見つけた場合、
// セキュリティ問題の可能性もあります。
//
// 機能性とセキュリティ上の懸念のバランスを取るため、go コマンドは
// デフォルトでは公開サーバーからコードをダウンロードする際に git と hg
// だけを使用します。ただし、GOPRIVATE 変数に一致するパッケージを
// ホストするものとして定義されるプライベートサーバーからコードを
// ダウンロードする場合は、既知の任意のバージョン管理システム
// (fossil、git、hg、svn) を使用します（'go help private' を参照）。
// Git と Mercurial だけを許可する理由は、この 2 つのシステムが、信頼できない
// サーバーのクライアントとして実行される際の問題に最も注意を払われてきたからです。
// それに対して Bazaar、Fossil、Subversion は主に信頼された認証済みの
// 環境で使われており、攻撃対象面としてはそれほど精査されていません。
//
// バージョン管理コマンドの制限は、コードをダウンロードするために直接
// バージョン管理アクセスを使用する場合にのみ適用されます。プロキシから
// モジュールをダウンロードする場合、goコマンドは代わりにプロキシプロトコルを
// 使用し、これは常に許可されます。デフォルトでは、goコマンドはGo
// モジュールミラー（proxy.golang.org）を公開パッケージに使用し、
// プライベートパッケージの場合のみバージョン管理にフォールバックするか、
// ミラーが公開パッケージを提供することを拒否する場合（通常は法的な理由で）です。
// したがって、クライアントはデフォルトでBazaar、Fossil、またはSubversion
// リポジトリから提供される公開コードにアクセスできます。これは
// Go モジュールミラーを使用するためです。これはカスタムサンドボックスを
// 使用してバージョン管理コマンドを実行するセキュリティリスクを取ります。
//
