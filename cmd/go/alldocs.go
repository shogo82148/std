...
// セキュリティ問題の可能性もあります。
//
// 機能とセキュリティの懸念のバランスを取るために、goコマンドは
// デフォルトでgitとhgのみを使用して公開サーバーからコードをダウンロードします。
// ただし、GOPRIVATE変数と一致するパッケージをホストしているプライベートサーバー
// （「go help private」参照）から定義されたコードをダウンロードするために、
// 既知のバージョン管理システム（bzr、fossil、git、hg、svn）を使用します。
// GitとMercurialのみを許可する理由は、これら2つのシステムが信頼されていない
// サーバーのクライアントとして実行されることに最も注意を払ってきたことです。
// 対照的に、Bazaar、Fossil、Subversionは主に信頼できる
// 認証環境で使用されており、攻撃対象としてそれほどよく精査されていません。
//
