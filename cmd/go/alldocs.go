...
//
// テストバイナリをビルドする一環として、go testはパッケージとそのテストソースファイルに対してgo vetを実行し、
// 重大な問題を特定します。go vetが問題を見つけた場合、go testはそれを報告し、テストバイナリを実行しません。
// デフォルトのgo vetチェックのうち、高信頼のサブセットのみが使用されます。
// そのサブセットは次のとおりです：atomic、bool、buildtags、directive、errorsas、ifaceassert、nilfunc、printf、およびstringintconv。
// これらおよび他のvetテストのドキュメントは、"go doc cmd/vet"で確認できます。
// go vetの実行を無効にするには、-vet=offフラグを使用します。すべてのチェックを実行するには、-vet=allフラグを使用します。
//
...
// # Import path syntax
//
// インポートパスは、ローカルファイルシステムに保存されているパッケージを表します。
// 一般的に、インポートパスは、標準パッケージ（例：「unicode/utf8」）
// またはモジュール内にあるパッケージのいずれかを表します（詳細については「go help modules」を参照）。
//
// # Internal Packages
//
...
// パッケージが属するモジュールのパスで始まります。モジュールのパスは
// モジュールのソースコードを取得する場所を指定します。
//
...
//
// ./または../で始まるインポートパスは相対パスと呼ばれます。
// 相対パスをコマンドラインのショートハンドとして使用できます。
// "unicode"としてインポートされたコードを含むディレクトリで作業していて、
// "unicode/utf8"のテストを実行したい場合は、
// 完全パスを指定する代わりに"go test ./utf8"と入力できます。
// 同様に、逆の状況では"go test .."は"unicode/utf8"ディレクトリから"unicode"をテストします。
// "go test ./..."のように相対パターンも許可され、すべてのサブディレクトリをテストできます。
// パターン構文の詳細については「go help packages」を参照してください。
//
// それ以外の場合、インポートパスPはビルドリスト内の
// いずれかのモジュールで見つかるパッケージを表します。「ビルドリスト」は
// ビルドに使用されるモジュールバージョンのリストです。
// 詳細は https://go.dev/ref/mod#glos-build-list を参照してください。
//
// インポートパスが指定されていない場合、アクションは現在のディレクトリ内のパッケージに適用されます。
//
// goツールでビルドするパッケージに使用すべきではないパスの予約済み名がいくつかあります：
//
// - "main"は、スタンドアロンの実行可能ファイルのトップレベルのパッケージを示します。
//
// - "work"はメインモジュール（またはワークスペースモジュール）内のすべてのパッケージに展開されます。
//
// - "all"はメインモジュール（またはワークスペースモジュール）内のすべてのパッケージとそれらの
// 依存関係（それらのいずれかのテストに必要な依存関係を含む）に展開されます。
// レガシーGOPATHモードでは、"all"はすべてのGOPATHツリーで見つかるすべてのパッケージに展開されます。
//
...
//
// インポートパスが1つ以上の"..."ワイルドカードを含む場合、そのパスはパターンです。
// 各ワイルドカードは空の文字列やスラッシュを含む文字列を含む任意の文字列と一致できます。
// そのようなパターンはファイルシステムパスパターンの場合はパターンに一致する
// ディレクトリにあるすべてのパッケージに、それ以外の場合はビルドリスト
// モジュール内でパターンに一致するすべてのパッケージに展開されます。
// "..."ワイルドカードはファイルシステムパスパターンの場合、モジュール境界を越えません。
//
// 一般的なパターンをより便利にするために、2つの特殊なケースがあります。
// まず、パターンの末尾の/...は空の文字列と一致することができるため、
// net/...はnetとnet/httpのようなそのサブディレクトリ内のパッケージの両方と一致します。
// 次に、ワイルドカードを含むスラッシュ区切りのパターン要素は、vendoredパッケージの
// パス内の"vendor"要素の一致に参加しないため、./...は
// ./vendorまたは./mycode/vendorのサブディレクトリ内のパッケージと一致しませんが、
// ./vendor/...と./mycode/vendor/...は一致します。
// ただし、vendor という名前で自らコードを含むディレクトリは vendoredパッケージではないことに注意してください。
// cmd/vendorはvendorという名前のコマンドになり、パターンcmd/...はそれと一致します。
// vendoringについての詳細は https://go.dev/ref/mod#vendoring を、
// レガシーGOPATHモードでのvendoringについては golang.org/s/go15vendor を参照してください。
//
// インポートパスは、リモートリポジトリからダウンロードされるパッケージを指定することもできます。
// 詳細は 'go help importpath' を実行してください。
//
// プログラム内のすべてのパッケージは一意のインポートパスを持つ必要があります。
// 最初のパス要素にドットがないパスは標準ライブラリのために予約されているか、
// 「example」と「test」の場合は、チュートリアル、例、テストコードでの使用のために
// 予約されています。モジュールモードでは、標準ライブラリ外のすべてのインポートパスは
// モジュールパスで始まります。これはモジュールパスの最初の要素にドットが
// あるべきことを意味します。例えば、「github.com/user/repo」または
// 「example.com/project」のように。
//
...
//
// 特殊なケースとして、パッケージリストが単一のディレクトリからの.goファイルのリストである場合、
// コマンドはまさにそれらのファイルで構成される「command-line-arguments」という名前の
// 単一の合成パッケージに適用されます。
//
