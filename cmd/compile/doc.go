...
//
// 相対ファイル名は、ディレクティブを含むファイルのディレクトリからの相対として解釈されます。
// 絶対ファイル名は指定どおりに使用されます。以前のディレクティブ
// （空ファイル名形式 //line :line:col）から引き継がれたファイル名は、
// 文字どおり再利用され、再解決は行われません。
//
// Examples:
//
//	//line foo.go:10      （相対）ファイル名は foo.go で、次の行の行番号は 10 になります
//	//line ../foo.go:10   相対ファイル名はディレクティブ元のソースディレクトリを基準に解決されます
//	//line C:foo.go:10    ファイル名中のコロンは許可され、ここでのファイル名は C:foo.go、行番号は 10 です
//	//line  a:100 :10     ファイル名中の空白は許可され、ここでのファイル名は " a:100 "（引用符を除く）です
//	/*line :10:20*/x      x の位置は、現在のファイルの行番号 10、列番号 20 になります
//	/*line foo: 10 */     このコメントは無効な行ディレクティブ（行番号の前後に余分な空白）として認識されます
//
