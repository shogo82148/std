...
一部のGo型の値は、その型のゼロ値以外の場合、常にGoポインタを含みます。
これは、文字列、スライス、インターフェース、チャネル、マップ、および関数型に当てはまります。
ポインタ型はGoポインタまたはCポインタを保持することができます。
配列および構造体型は、その要素型に応じてGoポインタを含む場合と含まない場合があります。
以下のGoポインタに関する議論は、ポインタ型だけでなく、Goポインタを含む他の型にも適用されます。
