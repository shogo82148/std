...
注意: 一部のGo型の値は、その型のゼロ値以外の場合、常にGoポインタを含みます。
これは、インターフェース、チャネル、マップ、および関数型に当てはまります。
ポインタ型はGoポインタまたはCポインタを保持することができます。
配列、スライス、文字列、および構造体型は、その型と構築方法に応じて、
Goポインタを含む場合と含まない場合があります。
以下のGoポインタに関する議論は、ポインタ型だけでなく、
