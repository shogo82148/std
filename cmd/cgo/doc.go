...
クロスコンパイル時だけでなく、CC環境変数が設定されておらず、デフォルトの
Cコンパイラ（通常gccまたはclang）がシステムのPATHで見つからない場合にも、デフォルトで無効になります。
goツールを実行する際にCGO_ENABLED環境変数を設定することで、デフォルトを上書きできます。
cgoの使用を有効にするには1に設定し、無効にするには0に設定します。goツールは、
cgoが有効な場合にビルド制約「cgo」を設定します。特別なインポート「C」は、
ファイルが「//go:build cgo」とも記述されているかのように、「cgo」ビルド制約を意味します。
したがって、cgoが無効な場合、「C」をインポートするファイルはgoツールによってビルドされません。
...
[runtime/cgo.Handle] 型は、GoとCの間でGo値を安全に渡すために使用できます。
