...
//
// 他のゴルーチンがすでに1つ以上のリーダーとしてロックを保持している状態で [RWMutex.Lock] を呼び出すと、
// 同時に行われる [RWMutex.RLock] の呼び出しは、ライターがロックを取得して解放するまでブロックされます。
// これは、ロックが最終的にライターに利用可能になることを保証するためです。
// この動作により、再帰的な読み取りロックは禁止されます。
// [RWMutex.RLock] を [RWMutex.Lock] にアップグレードすることはできず、
// [RWMutex.Lock] を [RWMutex.RLock] にダウングレードすることもできません。
//
