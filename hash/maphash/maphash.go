...
// maphash パッケージは、バイト列と比較可能な値に対するハッシュ関数を提供します。
// また、ハッシュ関数とハッシュテーブルの間のインターフェースである [Hasher] も定義します。
//
// これらのハッシュ関数は、ハッシュテーブル、Bloom フィルタ、
// そして任意の文字列やバイト列を符号なし 64 ビット整数上の
// 一様分布に写像する必要があるその他のデータ構造を実装するために使うことを想定しています。
//
// ハッシュテーブルやデータ構造の各インスタンスは、それぞれ固有の [Seed] を使うべきです。
//
