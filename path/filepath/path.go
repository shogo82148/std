...
// Dir は、path の最後の要素を除いた残りを返します。通常は path のディレクトリです。
// 最後の要素を取り除いたあと、Dir は path に対して [Clean] を呼び出し、末尾の
// スラッシュを削除します。
// path が空の場合、Dir は "." を返します。
// path が区切り文字だけで構成されている場合、Dir は単一の区切り文字を返します。
// 返される path は、ルートディレクトリである場合を除き、末尾に区切り文字を含みません。

// Windows では、"C:" のようなボリューム名のみのパスに対して、Dir は "C:."
// を返します。これは C ドライブ上の現在のディレクトリです。ドライブのルート "C:\" を
// 得るには、[VolumeName] を区切り文字と組み合わせて使用してください。
