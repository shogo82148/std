...
// スキャンメソッドは、[Scan]、[Scanf]、または [Scanln] の引数として実装するものです。
...
// Scanlnは [Scan] に似ていますが、改行でスキャンを停止し、
...
// Sscanlnは [Sscan] に似ていますが、改行でスキャンを停止し、最後のアイテムの後には改行またはEOFが必要です。
...
// Fscanlnは [Fscan] に似ていますが、改行でスキャンを終了し、最後の項目の後には改行かEOFが必要です。
