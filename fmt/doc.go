...
出力先によって定義される4つの印刷関数ファミリーがあります。
[Print]、[Println]、[Printf] は [os.Stdout] に書き込みます。
[Sprint]、[Sprintln]、[Sprintf] は文字列を返します。
[Fprint]、[Fprintln]、[Fprintf] は [io.Writer] に書き込みます。
[Append]、[Appendln]、[Appendf] は出力をバイトスライスに追加します。

各ファミリー内の関数は、名前の末尾に従ってフォーマットを行います。
Print、Sprint、Fprint、Append は各引数にデフォルトフォーマットを使用し、
どちらも文字列でない場合はオペランド間にスペースを追加します。
Println、Sprintln、Fprintln、Appendln は常にスペースを追加し、改行を追加します。
Printf、Sprintf、Fprintf、Appendf は一連の「動詞」を使用してフォーマットを制御します。
...
整数：
	...
	%o	base 8
	%O	0o接頭辞付きの8進数
	%q	Go構文で安全にエスケープされたシングルクォート付きルーンリテラル
	%x	16進数。a-fには小文字を使用
	%X	16進数。A-Fには大文字を使用
...
[Printf]、[Sprintf]、[Fprintf]、[Appendf] では、各フォーマット動詞が呼び出し時に渡された
...
%!sは、障害が発生したときに使用していた印刷動詞を示すだけです。
ただし、パニックがError、String、またはGoStringメソッドのnilレシーバによって
引き起こされた場合、出力は装飾されていない文字列"<nil>"になります。

APIが文字列フォーマットを扱う多くのパッケージでは、
[Print]、[Printf]、[Println] に似た3つ組の関数を公開しています。
例えば、[log.Print]、[log.Printf]、[log.Println] は、
引数をこのパッケージ内の対応する関数へ転送します。
混乱を避けるため、他のラッパー関数もこのパッケージで確立された
命名規約および振る舞いの規約に従うべきです。
特に、1つの関数が引数の個数に基づいて
リテラル出力（[Print] のようなもの）と
書式付き出力（[Printf] のようなもの）を切り替えるべきではありません。
代わりに、それぞれ別の関数を提供してください。
