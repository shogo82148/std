...
2. オペランドがFormatterインターフェースを実装している場合、
...
Sscan、Sscanf、およびSscanlnは引数文字列から読み取ります。

Scan、Fscan、Sscanは、入力の改行をスペースとして扱います。

Scanln、Fscanln、およびSscanlnは、改行でスキャンを停止し、
...
ScanfやFscanfのように、Sscanfは入力全体を消費する必要はありません。
Sscanfが使用した入力文字列の量を回復する方法はありません。

注意: Fscanなどは、返された入力の1文字（rune）を読み取ることができます。
