...
const (
	// Headerは、[Marshal] の出力と一緒に使用するのに適した一般的なXMLヘッダーです。
...
// [Unmarshal] がXML要素を正しい構造体フィールドにマッチさせることができます。
...
// [Unmarshal] が属性を正しい構造体フィールドにマッチさせることができます。
// MarshalXMLAttrがゼロ属性 [Attr]{}を返す場合、出力には属性が生成されません。
// MarshalXMLAttrは、フィールドタグに"attr"オプションを持つ構造体フィールドのみで使用されます。
type MarshalerAttr interface {
	MarshalXMLAttr(name Name) (Attr, error)
}

// MarshalIndentは [Marshal] と同様に動作しますが、各XML要素は新しい
...
// [StartElement] と [EndElement] トークンが適切にマッチしていない場合、エラーを返します。
//
// EncodeTokenは [Encoder.Flush] を呼び出しません。なぜなら、通常これは [Encoder.Encode] や [Encoder.EncodeElement]
// （またはそれらの間に呼び出されるカスタム [Marshaler] のMarshalXML）のような大きな操作の一部であり、
