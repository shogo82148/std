...
構造体は、(フィールド番号, フィールド値) の組の列として送信されます。フィールド値は、
その型に対する標準のgobエンコーディングを使って再帰的に送信されます。フィールドが
その型のゼロ値である場合（配列を除く。上記参照）、またはゼロ値へのポインタである場合、
そのフィールドは送信から省略されます。フィールド番号は、エンコードされる構造体の型に
よって定義されます。エンコード対象の型の最初のフィールドがフィールド0、2番目がフィールド1、
というようになります。値をエンコードする際、効率化のためフィールド番号はデルタ
エンコーディングされ、フィールドは常にフィールド番号の昇順で送信されます。したがって
デルタ値は符号なしです。デルタエンコーディングの初期値ではフィールド番号を -1 に設定するため、
値 7 を持つ符号なし整数のフィールド0は、符号なしデルタ = 1、符号なし値 = 7、すなわち
(01 07) として送信されます。最後に、すべてのフィールド送信後、終端マークが構造体の終わりを
示します。このマークは delta=0 の値で、表現は (00) です。
