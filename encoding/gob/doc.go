...
構造体は（フィールド番号、フィールド値）ペアのシーケンスとして送信されます。フィールド値は、
その型の標準的なgobエンコーディングを使用して再帰的に送信されます。フィールドがその型の
ゼロ値を持つ場合（配列を除く；上記参照）、またはゼロ値へのポインタの場合、送信から省略されます。
フィールド番号は、エンコードされた構造体の型によって定義されます：エンコードされた型の
最初のフィールドはフィールド0、2番目はフィールド1、等々です。値をエンコードするとき、
効率性のためフィールド番号はデルタエンコードされ、フィールドは常にフィールド番号の昇順で
送信されます；そのため、デルタは符号なしです。デルタエンコーディングの初期化では、
フィールド番号を-1に設定するため、値7を持つ符号なし整数フィールド0は、符号なしデルタ = 1、
符号なし値 = 7、または（01 07）として送信されます。最後に、すべてのフィールドが送信された後、
終了マークが構造体の終わりを示します。そのマークはデルタ=0の値で、（00）という表現を持ちます。
