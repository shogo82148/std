...
// MatchCaseInsensitiveNamesは、JSONオブジェクトのメンバー名を
// Go構造体フィールドに対して大文字・小文字を区別しない名前一致で
// 対応付けることを指定します。
// 名前が複数のフィールドに一致する場合、名前が完全一致するフィールドを選び、
// そうでなければエラーを報告します。
// Go構造体フィールドに `case:strict` または `case:ignore` が明示されている場合は、
// このオプションの値に関係なく、常に大文字・小文字を区別する
// （または区別しない）名前一致を使用します。
//
// このオプションはマーシャルまたはアンマーシャルのいずれかに影響します。
//
// 名前を大文字・小文字を区別せずに一致させることで、
// 同じ名前の揺れが同じGo構造体フィールドに一致しうるため、
// 重複名の検出にも影響します（[jsontext.AllowDuplicateNames] がfalseであると仮定）。
// 例えばアンマーシャル時には、"foo" と "Foo" の両方が同じ
// Go構造体フィールドに一致し、そのため重複名とみなされることがあります。
// マーシャル時には通常、どの2つのGo構造体フィールドも完全一致する名前は一意なので、
// 同じGo構造体フィールドへアンマーシャルされるような形で
// シリアライズされることはありません。
// しかし、埋め込みフォールバックを使うと、埋め込みフォールバック内に
// Go構造体フィールド名にも一致する名前を含めることができ、
// その結果、重複名エラーになる可能性があります。
