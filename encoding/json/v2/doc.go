...
//     このオプションはアンマーシャル時には効果がありません。
//
//   - string: "string"オプションは、構造体フィールド値のマーシャルまたは
//     アンマーシャル時に [StringifyNumbers] を設定することを指定します。
//     これにより、数値型（または数値型へのポインタ）はJSON文字列内の
//     JSON数値としてエンコードされ、周囲に空白を含まないJSON数値を含む
//     JSON文字列からデコードされます。
//     "string"オプションは再帰的には適用されません。具体的には、`string`は
//     複合データ型（例: 配列、スライス、構造体、マップ、インターフェース）
//     の内部にあるbool、string、または数値型を文字列化しません。
//     このオプションを不正な型に適用すると、実行時エラーになります。
//     多くのJSONパーサは64ビット整数を正確に表現できないため、
