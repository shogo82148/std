...
//     アンマーシャルするときに [StringifyNumbers] を設定することを指定します。
//     これにより、通常は JSON 数値としてエンコードされる型は、
//     JSON 文字列の中にクォートされた JSON 数値としてエンコードされ、
//     さらに、その JSON 数値を含む JSON 文字列から、
//     前後の空白なしでデコードされるようになります。
//     "string" オプションは、Go 構造体フィールド値の最上位レベルにのみ適用されます。
//     JSON 数値としてエンコードされない型にこのオプションを適用すると、
//     実行時エラーが発生します。特に、複合 Go データ型（例: 配列、スライス、構造体、
//     マップ）のデフォルト表現に対しては、
//     その型の内部の JSON 数値そのものを変更するのではなく、
//     実行時エラーになります。
//     64 ビット整数を正確に表現できない JSON パーサーが多いため、
