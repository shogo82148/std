...
//
//   - case: アンマーシャル時、"case" オプションは、JSONオブジェクト名を
//     Go構造体フィールドのJSON名とどのように一致させるかを指定します。
//     このオプションは "case:value" の形のキーと値の組で指定し、
//     value には 'ignore' または 'strict' のいずれかを指定しなければなりません。
//     'ignore' 値は、大文字小文字を区別しない一致を指定し、
//     さらにダッシュとアンダースコアも無視します。複数のフィールドが一致する場合、
//     正確な名前一致を持つフィールドが選択され、そうでなければ、
//     アンマーシャル先の候補フィールド集合が曖昧であるためエラーが報告されます。
//     'strict' 値は、大文字小文字を区別する一致を指定します。
