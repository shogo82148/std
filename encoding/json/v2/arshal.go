...
// *Tを操作する関数やメソッドは、T型の値をエンコードする場合
// （そのアドレスを取得することによって）または非nilの*T値をエンコードする場合にのみ呼び出されます。
// Marshalは、これらの関数やメソッドを一貫して呼び出せるよう、
// 値が常にアドレス可能であることを保証します
// （必要に応じて値をコピーします）。性能のため、Marshalには
// 値への非nilポインタを渡すことが推奨されます。
//
...
//     [StringifyNumbers] が指定されている場合、または JSON オブジェクト名を
//     エンコードしている場合は、その JSON 数値は JSON 文字列の中にエンコードされます。
//     NaN または ±Inf をエンコードすると [SemanticError] になります。
//
...
// *Tを操作する関数やメソッドは、T型の値をデコードする場合
// （そのアドレスを取得することによって）または非nilの*T値をデコードする場合にのみ呼び出されます。
// Unmarshalは、これらの関数やメソッドを一貫して呼び出せるよう、
// 値が常にアドレス可能であることを保証します
// （必要に応じて値をコピーします）。
// ポインタレシーバの [Unmarshaler]、[UnmarshalerFrom]、または
// [encoding.TextUnmarshaler] メソッドを呼び出すために値を浅くコピーする必要がある場合、
// そのメソッドによって行われた変更は、デスティネーション値へ
// 浅いコピーとして書き戻されます。
//
...
//     [StringifyNumbers] が指定されている場合や JSON オブジェクト名をデコードする場合、
//     JSON 数値を含む JSON 文字列からデコードされなければなりません。
//     Go の浮動小数点型の表現をオーバーフローした場合は失敗します。
//     JSON には NaN や ±Inf のネイティブ表現がないため、
//     それらの値がデコード結果として得られることはありません。
//
