...
//
// JSON 数値が符号なし整数だけから成る制限された文法に一致しない場合、
// [strconv.ErrSyntax] に一致するエラーを報告します。
// JSON 数値が符号なし整数であっても uint64 の表現可能範囲外である場合、
// [strconv.ErrRange] に一致するエラーを報告します。
// エラーが報告された場合でも、妥当な値は返されます。
// 数値の小数部分は無視されます（ゼロ方向への切り捨て）。
// uint64 の表現を超える数値は、最も近い表現可能な値に飽和されます。
//
// トークン種別が JSON 数値でない場合はパニックになります。
