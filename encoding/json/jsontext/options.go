...
// Options は、[NewEncoder]、[Encoder.Reset]、[NewDecoder]、
// および [Decoder.Reset] に対して特定の機能を設定します。
// 各関数は可変長のオプション列を受け取り、後に設定されたオプションのプロパティが
// 先に設定されたプロパティの値を上書きします。
//
...
// 特別なケースとして、-0は0として正規化されます。
//
// JSON数値はIEEE 754倍精度浮動小数点数として扱われます。
// シリアライズ済みの単精度数値を正規化し、
// それを単精度数値として再度パースしても同じ値になることが期待できます。
// 数値が最大有限値である ±1.7976931348623157e+308 を超える場合、
// その値に飽和させたうえで、その値としてフォーマットされます。
//
// このオプションはエンコード時のみ有効で、デコード時には無視されます。
...
//
// false に設定した場合、出力は1行となり、
// 生成される空白文字は現在の [SpaceAfterColon] と [SpaceAfterComma] の値によって決まります。
//
// このオプションはエンコード時のみ有効で、デコード時には無視されます。
func Multiline(v bool) Options

// WithIndent は、エンコーダが複数行の出力を生成することを指定します。
// JSONオブジェクトや配列の各要素は、新しいインデント付きの行で開始され、
// インデントプリフィックス（[WithIndentPrefix] を参照）に続いて、
// ネストの深さに応じてインデント文字列が1回以上繰り返されます。
// インデント文字列はスペースまたはタブのみで構成されている必要があります。
//
// 特定のインデント文字列にこだわりがなく、
// 単にインデント付き出力を行いたいだけなら、代わりに [Multiline] を使用してください。
//
// このオプションはエンコード時のみ有効で、デコード時には無視されます。
// このオプションを指定すると [Multiline] もtrueになります。
func WithIndent(indent string) Options

// WithIndentPrefix は、エンコーダが複数行の出力を生成することを指定します。
// JSONオブジェクトや配列の各要素は、新しいインデント付きの行で開始され、
// インデントプリフィックスの後に、ネストの深さに応じてインデント文字列が1回以上繰り返されます
// （[WithIndent] を参照）。
