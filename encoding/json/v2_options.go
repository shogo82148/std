...
//
//   - JSON null 以外の値をアンマーシャルする場合、このオプションは、
//     配列要素、スライス要素、構造体フィールド（ただしマップ値は除く）、
//     ポインタ値、およびインターフェース値（非 nil ポインタの場合のみ）について、
//     元の Go 値にマージします。
//     スライスについては、元のスライス長を超える要素についても、既存のスライス要素の値にマージします。
//     元のスライス長が JSON 配列より長い場合は、それに合わせて切り詰められます。
//     これに対して、v2 のデフォルト動作では、構造体フィールド、マップ値、
//     ポインタ値、およびインターフェース値に対して Go 値へマージします。
//     一般に、v2 のセマンティクスは JSON オブジェクトをアンマーシャルする場合にマージし、
//     それ以外では元の値を置き換えます。
//
...
//
//   - [jsonv2.MatchCaseInsensitiveNames] を使ってアンマーシャルする際、
//     JSONオブジェクト名が複数のGo構造体フィールドに非完全一致する場合、
//     エラーは報告されず、代わりに最初に宣言されたフィールドが使用されます。
//     一方、v2のセマンティクスではランタイムエラーを報告します。
//
//   - アンマーシャル時、JSON入力の構文構造は、JSONデータをGo値へ
//     セマンティックにアンマーシャルする前に完全に検証されます。
//     実際には、これは構文エラーを含むJSON入力が対象のGo値に対して
//     いかなる変更も引き起こさないことを意味します。
//     一方、v2のセマンティクスではストリーミングデコードを行い、JSON入力を
//     対象のGo値へ段階的にアンマーシャルするため、構文エラーに遭遇した時点で
//     Go値が部分的に変更されている可能性があります。
//
