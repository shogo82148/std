...
//
// "for b.Loop() { ... }"ループの本体内では、関数呼び出しの引数と結果、
// およびループ内で代入された変数は生存状態に保たれ、コンパイラがループ本体を
// 完全に最適化で除去することを防ぎます。
// 現在、これはそのような変数をruntime.KeepAlive内在関数呼び出しで
// ラップするコンパイラ変換として実装されています。これは、ループの
// 中括弧の間に構文的に存在する文にのみ適用され、ループ条件は
// 正確に"b.Loop()"として記述される必要があります。
//
