...
// 返される文字列は、制御文字および [IsPrint] によって定義された印刷不可能な文字のために、
// Goエスケープシーケンス（\t、\n、\xFF、\u0100）を使用します。
...
// 返される文字列は、Unicodeのグラフィック文字（[IsGraphic] によって定義される）を変更せず、
...
// QuotedPrefixは、sのプレフィックスにある引用符で囲まれた文字列（[Unquote] で理解される形式）を返します。
...
// ” に対して Unquote は空文字列を返します。）
func Unquote(s string) (string, error)

// IsPrintは、文字がGoによって印刷可能と定義されているかどうかを報告します。
// これは、[unicode.IsPrint] と同じ定義で、文字、数字、句読点、記号、ASCIIスペースを含みます。
